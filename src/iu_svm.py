...
class SVMClassifier:
    def __init__(self, kernel=LinearKernel, C=1000.0):
        self.alphas = []
    ...
    def addToSetS(self, index):
            ...
            self.Q = np.array(
                [
                    [
                        -self.y[index]
                        * self.y[index]
                        * self.kernel(self.x[index], self.x[index]),
                        self.y[index],
                    ],
                    [self.y[index], 0],
                ],
        ...
        nu = np.array(
            [
                [self.y[index]]
                + [
                    self.y[s] * self.y[index] * self.kernel(self.x[index], self.x[s])
                    for s in self.S
                ]
            ],
            np.float32,
        ...
        # print(betas)
        k = (
            self.y[index] * self.y[index] * self.kernel(self.x[index], self.x[index])
            - nu.T @ self.Q @ nu
        )
        # print(k)
        self.Q = (
    ...
    def calculateBetas(self):
        nu = np.array(
            [
                [self.y[-1]]
                + [
                    self.y[s] * self.y[-1] * self.kernel(self.x[-1], self.x[s])
                    for s in self.S
                ]
            ],
            np.float32,
    ...
    def calculateGammas(self, betas):
        K_cc_cr = np.array(
            [
                [self.y[r] * self.y[-1] * self.kernel(self.x[-1], self.x[r])]
                for r in [-1] + self.E + self.O
            ],
            np.float32,
        )
        m = np.concatenate(
            [
                ...
                [
                    [
                        self.y[r] * self.y[s] * self.kernel(self.x[r], self.x[s])
                        for s in self.S
                    ]
                    for r in [-1] + self.E + self.O
                ],
    ...
    def calculateDelta(self, betas, gammas):
        ...
        for i in range(1, len(self.S) + 1):
            if betas[i] > EPS:
                ...
                deltaAmax.append(np.array([INF], np.float32))
        ...
        for i in range(len(self.E)):
            if gammas[i + 1] > EPS:
                ...
                deltas.append(np.array([INF], np.float32))
        shift = len(self.E)
